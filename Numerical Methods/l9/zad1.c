...
double metoda_potegowa(int size, double A[size][size], double *x, int iterations);
...
int main() {
    ...
    double lambda = metoda_potegowa(A1SIZE,A1,x1,iter);
    printf("macierz A1\n");
    ...
    lambda = metoda_potegowa(A2SIZE,A2,x2,iter);
    printf("macierz A2\n");
    printf("wartosc wlasna: %.4f\n", lambda);
    printf("wektor wlasny:\n");
...
double metoda_potegowa(int size, double A[size][size], double *x, int iterations) {
    ...
    for(int i=0;i<iterations;i++) {
        ...
        prev_r = r;
        r = (3*y[1]+5*y[0])/(3*x[1]+5*x[0]);      //r = fi(y)/fi(x)
